...
contract GnosisDAppNodeIncentiveV2SafeModuleSetup {
     ...
     * @param module_address The address of the SafeModule deployment
     * @param info SafeModule data
     */
    function setup(address module_address, GnosisDAppNodeIncentiveV2SafeModule.UserInfo calldata info) external {
        Safe(payable(address(this))).enableModule(module_address);
        GnosisDAppNodeIncentiveV2SafeModule(module_address).registerSafe(info);
    }
}
